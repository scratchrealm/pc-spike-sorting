...
def run_kilosort3(
    *,
    ...
    print('Checking for invalid params')
    ...
    if use_docker:
        if use_singularity:
            raise Exception("Cannot use both docker and singularity")
    ...
    print(f'Loading output (keep_good_only={keep_good_only})')
...
def _check_kilosort3_installed():
    ...
    if kilosort3_path is not None:
        raise Exception('We do not support the KILOSORT3_PATH method. This sorter must be run in a container which has the ks3 command.')
    # check if _compiled_name is in the path using the which command
    if shutil.which(_compiled_name) is None:
        raise Exception(
            f"Kilosort3 is not installed. No such command: {_compiled_name}."
        )
...
def _generate_channel_map_file(recording, sorter_output_folder):
    ...
    for i in range(nchan):
        print(f'  {channel_ids[i]}: {channel_map["xcoords"][0][i]}, {channel_map["ycoords"][0][i]}')
    print('')
